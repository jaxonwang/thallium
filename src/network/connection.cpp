...
string Connection::socket_to_string() const {
    return holding_socket->remote_endpoint().address().to_string() + ":" +
...
void Connection::when_header_received(const std::error_code &ec,
    ...
    if (ec) {
        if (ec.value() == asio::error::operation_aborted)
            ...
            TI_DEBUG(format("Socket {}: Read error: {}", socket_to_string(),
                            ec.message()));
            return;
        }
    }
    if (bytes_read < message::header_size) {
        TI_DEBUG(format("Peer {} closed.", socket_to_string()));
        connection_close();
    }
    header_parser();
}
